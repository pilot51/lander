...
/**
 * An example database that records the state of each purchase. You should use an obfuscator before
 * storing any information to persistent storage. The obfuscator should use a key that is specific
 * to the device and/or user. Otherwise an attacker could copy a database full of valid purchases
 * and distribute it to others.
 */
public class PurchaseDatabase {
	private static final String TAG = Main.TAG + "-PurchaseDatabase";
	...
	private static final String[] HISTORY_COLUMNS = {HISTORY_ORDER_ID_COL, HISTORY_PRODUCT_ID_COL,
		HISTORY_STATE_COL, HISTORY_PURCHASE_TIME_COL, HISTORY_DEVELOPER_PAYLOAD_COL};
	...
	private static final String[] PURCHASED_COLUMNS = {PURCHASED_PRODUCT_ID_COL,
		PURCHASED_QUANTITY_COL};
	...
	/**
	 * Inserts a purchased product into the database. There may be multiple rows in the table for
	 * the same product if it was purchased multiple times or if it was refunded.
	 * 
	 * @param orderId
	...
	/**
	 * Updates the quantity of the given product to the given value. If the given value is zero,
	...
	/**
	 * Adds the given purchase information to the database and returns the total number of times
	 * that the given product has been purchased.
	 * 
	 * @param orderId
	...
	public synchronized int updatePurchase(String orderId, String productId,
		...
		Cursor cursor =
			mDb.query(PURCHASE_HISTORY_TABLE_NAME, HISTORY_COLUMNS, HISTORY_PRODUCT_ID_COL + "=?",
	...
	public Cursor queryAllPurchasedItems() {
		return mDb.query(PURCHASED_ITEMS_TABLE_NAME, PURCHASED_COLUMNS, null, null, null, null,
			null);
	}
	...
	private class DatabaseHelper extends SQLiteOpenHelper {
		...
		private void createPurchaseTable(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + PURCHASE_HISTORY_TABLE_NAME + "(" + HISTORY_ORDER_ID_COL
				+ " TEXT PRIMARY KEY, " + HISTORY_STATE_COL + " INTEGER, " + HISTORY_PRODUCT_ID_COL
				+ " TEXT, " + HISTORY_DEVELOPER_PAYLOAD_COL + " TEXT, " + HISTORY_PURCHASE_TIME_COL
				+ " INTEGER)");
			db.execSQL("CREATE TABLE " + PURCHASED_ITEMS_TABLE_NAME + "("
				+ PURCHASED_PRODUCT_ID_COL + " TEXT PRIMARY KEY, " + PURCHASED_QUANTITY_COL
				+ " INTEGER)");
		}
	}
}

...
public class Base64 {
	 ...
	 */
	private final static byte[] ALPHABET = {(byte)'A', (byte)'B', (byte)'C', (byte)'D', (byte)'E',
	 ...
	 */
	private final static byte[] WEBSAFE_ALPHABET = {(byte)'A', (byte)'B', (byte)'C', (byte)'D',
	...
	public static String
		encode(byte[] source, int off, int len, byte[] alphabet, boolean doPadding) {
	 ...
	 */
	public static byte[]
		encode(byte[] source, int off, int len, byte[] alphabet, int maxLineLength) {
		...
		for (; d < len2; d += 3, e += 4) {
			...
			int inBuff =
				((source[d + off] << 24) >>> 8) | ((source[d + 1 + off] << 24) >>> 16)
	...
	public static byte[] decode(byte[] source, int off, int len, byte[] decodabet)
						...
						if (b4Posn == 0 || b4Posn == 1) {
							...
							throw new Base64DecoderException(
								"padding byte '=' falsely signals end of encoded value "
									+ "at offset " + i);
						} else if (lastByte != EQUALS_SIGN && lastByte != NEW_LINE) { throw new Base64DecoderException(
		...
		if (b4Posn != 0) {
			if (b4Posn == 1) { throw new Base64DecoderException(
